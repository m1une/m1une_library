...
data:
  ...
  bundledCode: "#line 1 \"verify/graph/tree/vertex_get_range_contour_add_on_tree.test.cpp\"\
    ...
    \ % 21) - 10;\n        }\n\n        m1une::tree::VertexGetRangeContourAdd<long\
    ...
    \ right);\n                long long delta = static_cast<long long>(random() %\
    \ 21) - 10;\n                contour.apply(vertex, left, right, delta);\n    \
    \            for (int other = 0; other < n; other++) {\n                    if\
    \ (left <= distance[vertex][other] &&\n                        distance[vertex][other]\
    \ < right) {\n                        value[other] += delta;\n               \
    \     }\n                }\n            } else if (type == 1) {\n            \
    ...
    \ % std::uint64_t(n));\n            if (random() & 1) {\n                int left\
    \ = int(random() % std::uint64_t(n + 3));\n                int right = int(random()\
    \ % std::uint64_t(n + 3));\n                if (right < left) std::swap(left,\
    \ right);\n                std::uint64_t update = random();\n                contour.apply(vertex,\
    \ left, right, update);\n                for (int other = 0; other < n; other++)\
  ...
  verifiedWith: []
documentation_of: verify/graph/tree/vertex_get_range_contour_add_on_tree.test.cpp
layout: document
redirect_from:
- /verify/verify/graph/tree/vertex_get_range_contour_add_on_tree.test.cpp
- /verify/verify/graph/tree/vertex_get_range_contour_add_on_tree.test.cpp.html
