...
void randomized_test() {
    ...
    for (int trial = 0; trial < 250; trial++) {
        ...
        m1une::tree::VertexAddRangeContourSum<long long> contour(graph, value);
        ...
        for (int query = 0; query < 250; query++) {
            ...
            if (type == 0) {
                ...
                assert(contour.sum(vertex, left, right) == expected);
            }
        }
    }
...
void xor_group_test() {
    ...
    for (int trial = 0; trial < 120; trial++) {
        ...
        m1une::tree::VertexApplyRangeContourProduct<Group> contour(
            graph,
            value
        );
...
int main() {
    ...
    m1une::tree::VertexAddRangeContourSum<long long> contour(graph, initial);
    ...
    while (query_count--) {
        ...
        if (type == 0) {
            ...
            int left, right;
            std::cin >> left >> right;
            std::cout << contour.sum(vertex, left, right) << '\n';
        }
    }
}
