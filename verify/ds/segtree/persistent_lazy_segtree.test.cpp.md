...
data:
  ...
  bundledCode: "#line 1 \"verify/ds/segtree/persistent_lazy_segtree.test.cpp\"\n#define\
    ...
    \ = versions[version]->apply(left, right, std::pair<mint, mint>(multiplier, addition));\n\
    \        } else if (type == 1) {\n            int source;\n            fast_input\
    \ >> source >> left >> right;\n            source++;\n            versions[query\
    \ + 1] = versions[version]->copy_range_from(*versions[source], left, right);\n\
    \        } else {\n            fast_input >> left >> right;\n            fast_output\
    \ << versions[version]->prod(left, right).sum << '\\n';\n        }\n    }\n}\n"
