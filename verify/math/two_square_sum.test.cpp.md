...
data:
  ...
  bundledCode: "#line 1 \"verify/math/two_square_sum.test.cpp\"\n#define PROBLEM \"\
    https://judge.yosupo.jp/problem/two_square_sum\"\n\n#include <algorithm>\n#include\
    \ <cassert>\n#include <cstdint>\n#include <iostream>\n#include <utility>\n#include\
    ...
    \    }\n    return true;\n}\n\n}  // namespace math\n}  // namespace m1une\n\n\
    \n#line 12 \"verify/math/two_square_sum.test.cpp\"\n\nstd::vector<std::pair<uint64_t,\
    \ uint64_t>> brute_two_square_sum(uint64_t value) {\n    std::vector<std::pair<uint64_t,\
    \ uint64_t>> result;\n    for (uint64_t a = 0; a <= value / std::max<uint64_t>(a,\
    \ 1); a++) {\n        const uint64_t remaining = value - a * a;\n        const\
    \ uint64_t b = m1une::math::isqrt(remaining);\n        if (b * b == remaining)\
    \ result.emplace_back(a, b);\n    }\n    return result;\n}\n\nvoid check_two_square_sum(uint64_t\
    \ value) {\n    std::vector<std::pair<uint64_t, uint64_t>> expected =\n      \
    \  brute_two_square_sum(value);\n    std::vector<std::pair<uint64_t, uint64_t>>\
    \ actual =\n        m1une::math::two_square_sum(value);\n\n    std::sort(expected.begin(),\
    \ expected.end());\n    std::sort(actual.begin(), actual.end());\n    assert(actual\
    \ == expected);\n    assert(m1une::math::represent_as_two_square_sum(value) ==\
    ...
    \ (uint64_t value = 0; value <= 1000; value++) {\n        check_two_square_sum(value);\n\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/two_square_sum\"\n\n#include\
    ...
    \ \"math/two_square_sum.hpp\"\n\nstd::vector<std::pair<uint64_t, uint64_t>> brute_two_square_sum(uint64_t\
    \ value) {\n    std::vector<std::pair<uint64_t, uint64_t>> result;\n    for (uint64_t\
    \ a = 0; a <= value / std::max<uint64_t>(a, 1); a++) {\n        const uint64_t\
  ...
  verifiedWith: []
documentation_of: verify/math/two_square_sum.test.cpp
layout: document
redirect_from:
- /verify/verify/math/two_square_sum.test.cpp
- /verify/verify/math/two_square_sum.test.cpp.html
