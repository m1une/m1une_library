...
data:
  ...
  bundledCode: "#line 1 \"verify/math/fps/inv_of_formal_power_series_sparse.test.cpp\"\
    ...
    \        return *this;\n    }\n};\n\n}  // namespace utilities\n}  // namespace\
    \ m1une\n\n\n#line 5 \"verify/math/fps/inv_of_formal_power_series_sparse.test.cpp\"\
    \n#include <limits>\n#include <random>\n\n#line 1 \"math/fps/sparse_formal_power_series.hpp\"\
    \n\n\n\n#line 6 \"math/fps/sparse_formal_power_series.hpp\"\n#include <optional>\n\
    #line 8 \"math/fps/sparse_formal_power_series.hpp\"\n#include <vector>\n\n#line\
    ...
    \ = (root * root).pre(degree);\n        auto sparse_root = m1une::fps::sparse_fps_sqrt(to_sparse(square),\
    \ degree);\n        assert(sparse_root.has_value());\n        assert(((*sparse_root)\
    \ * (*sparse_root)).pre(degree) == square);\n    }\n\n    Sparse empty;\n    Fps\
    \ one(8);\n    one[0] = 1;\n    assert(m1une::fps::sparse_fps_exp(empty, 8) ==\
    \ one);\n    assert(m1une::fps::sparse_fps_pow(empty, 0, 8) == one);\n    assert(m1une::fps::sparse_fps_sqrt(empty,\
    \ 8) == Fps(8));\n\n    Sparse shifted;\n    shifted.emplace_back(1, 1);\n   \
    \ assert(m1une::fps::sparse_fps_pow(\n               shifted, std::numeric_limits<long\
    \ long>::max(), 8) == Fps(8));\n    assert(!m1une::fps::sparse_fps_sqrt(shifted,\
    \ 8).has_value());\n}\n\nint main() {\n    m1une::utilities::FastInput fast_input;\n\
    \    m1une::utilities::FastOutput fast_output;\n\n    randomized_test();\n\n \
    \   int n, k;\n    fast_input >> n >> k;\n    Sparse f(k);\n    for (auto& [degree,\
    \ coefficient] : f) fast_input >> degree >> coefficient;\n    auto result = m1une::fps::sparse_fps_inv(f,\
    \ n);\n    for (int i = 0; i < n; i++) {\n        if (i) fast_output << ' ';\n\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/inv_of_formal_power_series_sparse\"\
    ...
    \ < int(f.size()); i++) {\n        if (f[i] != mint(0)) result.emplace_back(i,\
    \ f[i]);\n    }\n    return result;\n}\n\nvoid randomized_test() {\n    std::mt19937\
    \ rng(271828182);\n    for (int iteration = 0; iteration < 80; iteration++) {\n\
    \        const int degree = int(rng() % 28) + 2;\n        Fps f(degree);\n   \
    \     f[0] = mint(rng());\n        if (f[0] == mint(0)) f[0] = 1;\n        for\
    \ (int i = 1; i < degree; i++) {\n            if (rng() % 4 == 0) f[i] = mint(rng());\n\
    \        }\n        Sparse sparse = to_sparse(f);\n        assert(m1une::fps::sparse_fps_inv(sparse,\
    \ degree) == f.inv(degree));\n\n        f[0] = 1;\n        sparse = to_sparse(f);\n\
    \        assert(m1une::fps::sparse_fps_log(sparse, degree) == f.log(degree));\n\
    \n        f[0] = 0;\n        sparse = to_sparse(f);\n        assert(m1une::fps::sparse_fps_exp(sparse,\
    \ degree) == f.exp(degree));\n\n        const long long exponent = rng() % 7;\n\
  ...
  verifiedWith: []
documentation_of: verify/math/fps/inv_of_formal_power_series_sparse.test.cpp
layout: document
redirect_from:
- /verify/verify/math/fps/inv_of_formal_power_series_sparse.test.cpp
- /verify/verify/math/fps/inv_of_formal_power_series_sparse.test.cpp.html
