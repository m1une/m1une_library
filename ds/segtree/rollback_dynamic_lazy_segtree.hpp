#ifndef M1UNE_DS_SEGTREE_ROLLBACK_DYNAMIC_LAZY_SEGTREE_HPP
#define M1UNE_DS_SEGTREE_ROLLBACK_DYNAMIC_LAZY_SEGTREE_HPP 1

#include <concepts>
#include <type_traits>
#include <utility>

#include "../detail/rollback_persistent_base.hpp"
#include "persistent_dynamic_lazy_segtree.hpp"

namespace m1une {
namespace ds {

template <m1une::acted_monoid::IsActedMonoid ActedMonoid, std::integral Index = long long>
    requires(!std::same_as<std::remove_cv_t<Index>, bool>)
struct RollbackDynamicLazySegtree
    : detail::RollbackPersistentBase<PersistentDynamicLazySegtree<ActedMonoid, Index>> {
    using T = typename ActedMonoid::value_type;
    using F = typename ActedMonoid::operator_type;

   private:
    using Persistent = PersistentDynamicLazySegtree<ActedMonoid, Index>;
    using Base = detail::RollbackPersistentBase<Persistent>;

   public:
    using Base::Base;

    void set(Index pos, T value) {
        Base::commit(Base::persistent().set(pos, std::move(value)));
    }
    void set_inplace(Index pos, T value) { set(pos, std::move(value)); }

    void apply(Index pos, const F& f) {
        Base::commit(Base::persistent().apply(pos, f));
    }
    void apply(Index left, Index right, const F& f) {
        Base::commit(Base::persistent().apply(left, right, f));
    }
    void apply_inplace(Index pos, const F& f) { apply(pos, f); }
    void apply_inplace(Index left, Index right, const F& f) { apply(left, right, f); }
};

}  // namespace ds
}  // namespace m1une

#endif  // M1UNE_DS_SEGTREE_ROLLBACK_DYNAMIC_LAZY_SEGTREE_HPP
