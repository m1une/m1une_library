#ifndef M1UNE_FPS_INTERNAL_NTT998_FAST_HPP
#define M1UNE_FPS_INTERNAL_NTT998_FAST_HPP 1
...
// Fixed-modulus AVX2 kernel.  It evaluates only the upper radix-4 NTT layers
// and multiplies the remaining degree-8/16 residues directly modulo x^k-w.
    ...
    void transform_forward(int lg, u32* data) const {
        ...
        if (L < lg) {
            ...
            for (int i = 1; i < (1 << (lc * 2 - 2)); i++) {
                ...
                for (int k = s; k >= 0; k--) {
                    transform_aux<false>(2 * k + L, i * (1 << (L + 2)), data,
                                         wi_data[k], mts);
                }
            }
        }
