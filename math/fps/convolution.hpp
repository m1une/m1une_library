        ...
        while (phase < height) {
            ...
            for (int block = 0; block < (1 << phase); block++) {
                ...
                for (int i = 0; i < width; i++) {
                    ...
                    const uint64_t a2 = uint64_t(a[offset + i + 2 * width].val()) * twiddle2.val();
                    const uint64_t a3 = uint64_t(a[offset + i + 3 * width].val()) * twiddle3.val();
                    const uint64_t a1na3i = uint64_t(Mint(a1 + mod2 - a3).val()) * imaginary.val();
        ...
        while (phase > 0) {
            if (phase == 1) {
                ...
                for (int block = 0; block < (1 << (phase - 1)); block++) {
                    ...
                    if (block + 1 != (1 << (phase - 1))) twiddle *= roots.inverse_rate[__builtin_ctz(~uint32_t(block))];
                }
                phase--;
                continue;
            }
            ...
            for (int block = 0; block < (1 << (phase - 2)); block++) {
                ...
                for (int i = 0; i < width; i++) {
                    ...
                    const uint64_t a2na3i = uint64_t(Mint((Mint::mod() + a2 - a3) * inverse_imaginary.val()).val());
                    a[offset + i] = Mint(a0 + a1 + a2 + a3);
                    a[offset + i + width] = Mint((a0 + Mint::mod() - a1 + a2na3i) * twiddle.val());
                    a[offset + i + 2 * width] = Mint((a0 + a1 + 2ULL * Mint::mod() - a2 - a3) * twiddle2.val());
                    a[offset + i + 3 * width] = Mint((a0 + Mint::mod() - a1 + Mint::mod() - a2na3i) * twiddle3.val());
                }
...
template <class Mint>
__attribute__((target("avx2,bmi"), hot)) std::vector<Mint> convolution_998244353_simd(const std::vector<Mint>& a,
                                                                                      const std::vector<Mint>& b) {
    const int result_size = int(a.size() + b.size() - 1);
    int n = 1;
    while (n < result_size) n <<= 1;
    auto* transformed_a = static_cast<uint32_t*>(::operator new[](sizeof(uint32_t) * n, std::align_val_t(32)));
    auto* transformed_b = static_cast<uint32_t*>(::operator new[](sizeof(uint32_t) * n, std::align_val_t(32)));
    ...
    fast998_v2::vector_dit<true>(reinterpret_cast<__m256i*>(transformed_a), vector_size, &transform);
...
std::vector<Mint> convolution_ntt(const std::vector<Mint>& a, const std::vector<Mint>& b) {
    ...
    if constexpr (Mint::mod() == 998244353) {
        if (n >= 64 && __builtin_cpu_supports("avx2")) return internal::convolution_998244353_simd(a, b);
    }
#endif
...
std::vector<Mint> convolution(const std::vector<Mint>& a, const std::vector<Mint>& b) {
    ...
    [[maybe_unused]] const unsigned __int128 coefficient_bound =
        static_cast<unsigned __int128>(std::min(a.size(), b.size())) * (Mint::mod() - 1) * (Mint::mod() - 1);
    ...
    static const uint64_t mod1_mod2_mod3 = mod1_mod3 * (Mint2::mod() % Mint3::mod()) % Mint3::mod();
    ...
    for (int i = 0; i < result_size; i++) {
        ...
        const uint64_t first =
            (r2 + Mint2::mod() - r1 % Mint2::mod()) % Mint2::mod() * inverse_mod1_mod2 % Mint2::mod();
        const uint64_t combined_mod3 = (r1 % Mint3::mod() + mod1_mod3 * (first % Mint3::mod())) % Mint3::mod();
        const uint64_t second =
            (r3 + Mint3::mod() - combined_mod3) % Mint3::mod() * inverse_mod1_mod2_mod3 % Mint3::mod();
